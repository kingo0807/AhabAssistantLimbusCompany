...
class Automation(metaclass=SingletonMeta):
    ...
    def _remember_screenshot(self, screenshot: Image | None, timestamp: float | None = None) -> None:
        if screenshot is None:
            return
        self._latest_screenshot = screenshot
        self._latest_screenshot_monotonic = time.monotonic() if timestamp is None else timestamp
    ...
    def mouse_action_with_pos(
        ...
        # 计算传入的位置
        interaction_gate = getattr(self, "_interaction_gate", None)
        if interaction_gate is not None:
            interaction_gate.wait(timeout=GATE_WAIT_TIMEOUT)
    ...
    def take_screenshot(self, gray: bool = True, interval: float | None = None) -> Image | None:
        ...
        while True:
            try:
                ...
                # 测试替身可能绕过 __init__ 创建实例，因此缺锁时退化为无操作上下文。
                input_lock = getattr(self, "_input_lock", None)
                screenshot_lock = getattr(self, "_screenshot_lock", None)
                with (input_lock or nullcontext()), (screenshot_lock or nullcontext()):
                    result = ScreenShot.take_screenshot(gray)
                    if result:
                        screenshot_time = time.monotonic()
                        self._remember_screenshot(result, screenshot_time)
